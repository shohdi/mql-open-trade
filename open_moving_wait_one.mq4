...

{
   ...
   double vbidSell    = MarketInfo(_Symbol,MODE_BID);

   double vaskBuy    = MarketInfo(_Symbol,MODE_ASK);
   ...
   {
      if(tradeDirection == TRADE_TYPE::BUY)
      {
          if(vaskBuy > currentHigh)
            {
               currentHigh = vaskBuy;
               if(openTicket != 0)
               {
                  if(OrderSelect(openTicket,SELECT_BY_TICKET)==true)
                  {
                     OrderDelete(openTicket);
                     openTicket = 0;
                  }
               }
               
               openTicket = calcAndOpenTrade(tradeDirection);
            }
      }
      else
      {
         if(vbidSell < currentLow)
            {
               currentLow = vbidSell;
               if(openTicket != 0)
               {
